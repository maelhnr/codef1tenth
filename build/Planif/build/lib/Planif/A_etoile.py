...
class Astar(Node):
    def __init__(self, map_location = './src/Planif/Planif/example.pgm', map_info_location =  "./src/Planif/Planif/map.yaml", pos_topic = '/odom', waypoint_topic='/waypoints'):
        super().__init__('A_star')

        self.goal_poses = Path() # liste des waypoints
        # /!\ A CHANGER : OBTENIR POINT D'ORIGINE DU REPERE AVEC SLAM
        self.map_info = map_info_location
        self.resolution_carte = 0.05 # 1 pixel = 5 cm
        self.init_pos = (1374, 1464) # position initiale du robot
        ...
        #self.pos_sub = self.create_subscription(Odometry, pos_topic, self.create_waypoints, 10)
        self.waypoint_pub = self.create_publisher(Path, waypoint_topic, 10)

        # Création de la map
        self.circuit = Image.open(map_location) # Image simulant une map complète donnée par le SLAM
    ...
    def create_waypoints(self, data, csv_plot = False):
        ...
        # /!\ A CHANGER : OBTENIR POSITION DE LA VOITURE AVEC LE NOEUD SLAM
